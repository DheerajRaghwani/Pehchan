...
const apiUrl = "https://localhost:7003/api/Motherschemerecord";
...
function renderTable() {
    ...
    pageData.forEach((m, index) => {
        html += `<tr data-rchid="${m.rchid}">
            <td>${m.sno}</td>
            <td contenteditable>${m.district}</td>
            <td contenteditable>${m.healthBlock}</td>
            <td contenteditable>${m.healthFacility}</td>
            <td contenteditable>${m.healthSubFacility}</td>
            <td contenteditable>${m.village}</td>
            <td>${m.rchid}</td>
            <td contenteditable>${m.motherName}</td>
            <td contenteditable>${m.husbandName}</td>
            <td contenteditable>${m.mobileof}</td>
            <td contenteditable>${m.mobileNo}</td>
            <td contenteditable>${m.ageAsPerRegistration}</td>
            <td contenteditable>${formatDate(m.motherBirthDate)}</td>
            <td contenteditable>${m.address}</td>
            <td contenteditable>${m.anmname}</td>
            <td contenteditable>${m.ashaname}</td>
            <td contenteditable>${formatDate(m.registrationDate)}</td>
            <td contenteditable>${formatDate(m.lmd)}</td>
            <td contenteditable>${formatDate(m.edd)}</td>
            <td>
                <button class="btn-update" onclick="updateMother(this)">Update</button>
                <button class="btn-delete" onclick="deleteMother(${m.rchid})">Delete</button>
            </td>
        </tr>`;
    });
...
// -------------------- Count Functions --------------------
async function loadmby() { await loadCount("mby", "totalmby"); }
async function loadjsy() { await loadCount("jsy", "totaljs"); }
async function loadjssy() { await loadCount("jssy", "totaljss"); }
async function loadpmmvy() { await loadCount("pmmvy", "totalmv"); }
async function loadmmjy() { await loadCount("mmjy", "totalmm"); }
...
document.getElementById("totalmby").addEventListener("click", () => filterByScheme("mby"));
document.getElementById("totaljs").addEventListener("click", () => filterByScheme("jsy"));
document.getElementById("totaljss").addEventListener("click", () => filterByScheme("jssy"));
document.getElementById("totalmv").addEventListener("click", () => filterByScheme("pmmvy"));
document.getElementById("totalmm").addEventListener("click", () => filterByScheme("mmjy"));
...
function mapMotherData(data) {
    return data.map(m => ({
        ...
        ageAsPerRegistration: m.ageAsPerRegistration ?? m.AgeAsPerRegistration ?? "",
...
async function deleteMother(rchid) {
    ...
    try {
        const token = localStorage.getItem("jwtToken");
        const res = await fetch(`${apiUrl}/delete/${rchid}`, {   // <-- note /delete/
            method: "DELETE",
            headers: { "Authorization": `Bearer ${token}` }
        });
...
// -----NOT GET
document.getElementById("certificate1").addEventListener("change", async e => {
