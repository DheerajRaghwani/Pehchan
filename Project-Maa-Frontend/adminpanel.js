// Base API URL
const apiUrl = "https://localhost:7003/api/Userlogin";
  ...
  try {
    ...
    users.forEach(u => {
      html += `<tr>
        ...
        <td><button onclick="deleteUser(${u.id})" style="background:#ef4444;padding:6px 8px;border-radius:6px">Delete</button></td>
      </tr>`;
    });
    html += '</tbody></table>';
    ...
    console.error(err);
    userListWrap.innerHTML = '<div class="muted">Error loading users. Check API/CORS or token.</div>';
  }
}
...
async function deleteUser(id) {
  ...
  try {
    const response = await apiFetch(`${apiUrl}/Delete/${id}`, { method: 'DELETE' });
