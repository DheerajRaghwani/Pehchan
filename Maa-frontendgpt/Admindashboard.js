...
const apiUrl = "https://localhost:7003/api/Childrecord";
...
async function deleteChild(button) {
    const row = button.closest("tr");
    const rchid = row.dataset.rchid;
    if (!confirm(`Are you sure you want to delete record with RCHID ${rchid}?`)) return;
...
// -------------------- Pagination --------------------
function prevPage() { if (currentPage>1){currentPage--; renderChildren();} }
function nextPage() { const totalPages=Math.ceil(filteredData.length/pageSize); if(currentPage<totalPages){currentPage++; renderChildren();} }
...
function mapChildrenData(data) {
    return data.map(c => ({
        ...
        ageasperRegistration: c.ageasperRegistration ?? c.AgeAsPerRegistration ?? "",
        ...
        casteCertificateNumber: c.casteCertificateNumber ?? c.CasteCertificateNumber ?? "",
...
document.addEventListener("DOMContentLoaded", () => {
	...
	document.getElementById("totalbirth").addEventListener("click", loadBirthCertificateTable);
	document.getElementById("totalcaste").addEventListener("click", loadCasteCertificateTable);
	document.getElementById("totalration").addEventListener("click", loadRationCardTable);
	document.getElementById("totalaadhar").addEventListener("click", loadAadharCardTable);
	document.getElementById("totalayushman").addEventListener("click", loadAyushmanCardTable);
});
...
// Function to filter table based on selected village and block
function filterByVillageAndBlock() {
    const selectedVillage = villageSelect.value.trim().toLowerCase();
    ...
    filteredData = childrenData.filter(c => {
        ...
        const block = (c.healthBlock ?? "").trim().toLowerCase();

        let villageMatch = selectedVillage ? village === selectedVillage : true;
        let blockMatch = selectedBlock ? block === selectedBlock : true;

        return villageMatch && blockMatch;
    });

    currentPage = 1; // reset to first page
    renderChildren(); // re-render table and update total count
}
// Event listeners for dropdowns
villageSelect.addEventListener("change", filterByVillageAndBlock);
